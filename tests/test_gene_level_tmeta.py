...
import guide_pmd.gene_level_tmeta as mod


def test_compute_gene_tmeta_downweights_outlier():
    ...
    config = mod.GeneTMetaConfig(scope="flagged", min_guides=3, nu=4.0)
    gene, guides = mod.compute_gene_tmeta(
        per_guide_ols,
        annotation_table,
    ...
    outlier_w = float(guides.loc[guides["guide_id"] == "g4", "tmeta_weight"].iloc[0])
    inlier_w = guides.loc[guides["guide_id"] != "g4", "tmeta_weight"].to_numpy(dtype=float)
    assert np.isfinite(outlier_w)
    assert np.all(np.isfinite(inlier_w))
    assert outlier_w < float(np.min(inlier_w))


def test_compute_gene_tmeta_insufficient_guides_reports_error():
    per_guide_ols = pd.DataFrame(
        [
...
def test_compute_gene_tmeta_scope_flagged_skips_unflagged_genes():
    per_guide_ols = pd.DataFrame(
        [
            {"guide_id": "g1", "focal_var": "treatment", "beta": 0.5, "se": 0.2},
            {"guide_id": "g2", "focal_var": "treatment", "beta": 0.6, "se": 0.2},
